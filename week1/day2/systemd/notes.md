...
`systmectl enable/disable nginx.service`

### Check if service is active, enabled or failed
`systemctl is-active/is-enable/is-failed nginx.service`
...
### Reload systmemctl daemon to update all changed services
...
`systemctl reboot/rescue/poweroof`
...
### List all units
`systmectl list-units --all`

### Filter units by state
It wiil filter by 3 colums: LOAD, ACTIVE and SUB
`systemctl list-units --all --state=invactive`

### Filter units by type
`systmectl list-units -all --type=target`
